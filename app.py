...
def run_script3():
    try:
        ...
        with tempfile.NamedTemporaryFile(delete=False, mode='w') as cert_file, \
             ...
             tempfile.NamedTemporaryFile(delete=False, mode='w') as pass_file:

            cert_file.write(cert_content.strip())
            cert_file.flush()
            key_file.write(key_content.strip())
            key_file.flush()
            pass_file.write(os.getenv('PFX_PASSWORD'))
            pass_file.flush()
        ...
        try:
            ...
            result = subprocess.run(
                ["openssl", "pkcs12", "-export", "-out", pfx_file,
                 "-inkey", key_file.name, "-in", cert_file.name, "-passout", f"file:{pass_file.name}"],
                capture_output=True, text=True, check=True
            )
        except subprocess.CalledProcessError as e:
            error_output = e.stderr.lower()
            if "unable to load private key" in error_output:
                return jsonify({"error": "Invalid private key or mismatch with certificate."}), 400
                ...
                return jsonify({"error": "PFX creation failed."}), 400
        finally:
