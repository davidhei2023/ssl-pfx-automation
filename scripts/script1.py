...
def generate_certificate(fqdn):
    ...
    # Define file paths
    output_folder = f"/tmp/{sanitized_fqdn}-2025"
    key_file = f"{output_folder}/{sanitized_fqdn}-2025.key"
    csr_file = f"{output_folder}/{sanitized_fqdn}-2025.csr"
    dk_key_file = f"{output_folder}/DK{sanitized_fqdn}-2025.key"
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python3 script1.py <FQDN>", file=sys.stderr)
        sys.exit(1)
    fqdn = sys.argv[1]
